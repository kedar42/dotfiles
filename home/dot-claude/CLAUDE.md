...
**Work in the main conversation by default.** A feature in one repo, a fix, a refactor, a question: do it directly. Dispatch subagents only for 3+ independent slices, bulk output I never need to see, or when I ask for an independent read. How to run a fleet is the `dev-fleet` skill's job; how to review is `/review`'s.

**The Bash tool runs zsh on my Mac.** Any command with a loop, `$var` word-splitting, or a glob that may not match goes inside `bash <<'EOF' … EOF`. `timeout` does not exist here. After a destructive loop, count what it did before reporting success.
