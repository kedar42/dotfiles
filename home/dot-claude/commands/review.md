...
2. **Find** — dispatch `lens-reviewer` agents (sonnet) in parallel, one per applicable lens from its catalog (in `~/.claude/agents/lens-reviewer.md`). Always run the review-tier lenses (**ticket-alignment**, **smells/YAGNI/dead-code**, **codebase-consistency**, **correctness**); ADD the contextual ones whose trigger the diff hits — **layer-boundary** (persistence/data-layer, repository, controller, or frontend files changed) and **db-change-validation** (a migration/schema/model changed). This is the full set — both tiers. For a multi-repo PR add a cross-repo "is the backend change actually used by the frontend" reviewer.
