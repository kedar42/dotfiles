...
2. **Find** — dispatch reviewers in parallel: one `lens-reviewer` (sonnet) per quality lens (**ticket-alignment**, **smells/YAGNI/dead-code**, **codebase-consistency**, **correctness**, **ai-generation-tell**), PLUS — when the change is structural (persistence/data-layer, repository, controller, or frontend files, or a migration/schema/model changed) — one `standards-reviewer` (sonnet) for architectural integrity + maintainability. For a multi-repo PR add a cross-repo "is the backend change actually used by the frontend" reviewer.
