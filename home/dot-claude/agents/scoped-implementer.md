...
- **Do the work — don't pass it through.** Implement your slice yourself; do not spawn subagents. If the slice is too big or ambiguous to implement directly, stop and report back to the orchestrator with the concrete sub-slices you see — the orchestrator owns the fan-out.
...
- **Refactor to reuse, never patch.** Before writing, find the existing code that does 80% of your slice and extend or refactor it so both callers fit. A parallel helper, a copied block, or an `if` stacked on an existing path is a defect in your slice. Behaviour goes where its abstraction lives, not at the call site.
