---
name: lens-reviewer
description: Reviews a diff through ONE assigned quality lens (correctness, codebase-consistency, smells/YAGNI/dead-code, ticket-alignment, or AI-generation-tell). Read-only fan-out finder for a review wave. (Architectural integrity — layering, DB modeling, shortcuts — is `standards-reviewer`'s job, not a lens here.)
mode: subagent
...
The quality lenses. Architectural integrity / maintainability (layering, DB modeling, shortcut-hunting) is a separate perspective owned by the `standards-reviewer` agent — dispatch that when a change is structural, not a lens here.

| lens | principle | trigger |
|---|---|---|
