...
- contextual safety check (`standards-reviewer`, sonnet) — see below; NOT a full review
...
`scoped-implementer` · `lens-reviewer` · `standards-reviewer` · `adversarial-verifier` · `fix-verifier` · `comment-auditor` · `investigator`.
...
**Contextual safety check — NOT a full review.** After implementing, if the diff is structural — a data/persistence layer, repository, controller, or frontend source file changed, or a migration/schema/model changed — run ONE `standards-reviewer` (sonnet) over it. If nothing structural changed, skip it entirely. This is a targeted architectural/maintainability gate to catch expensive-to-unwind mistakes early — deliberately NOT the full lens sweep. The full sweep (all quality lenses + standards-reviewer + adversarial-verify) is `/review`, run separately when you actually want a PR review.
