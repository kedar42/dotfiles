...
**Default answer: no.** A change in one repo, even multi-file, is implemented **inline in this conversation** — no fan-out, no gates, no ceremony. The broad trigger is deliberate: this skill fires on any "implement/start work" phrasing precisely so it can make THIS call.
...
**Do Step 0 in plan mode when the feature is non-trivial or ambiguous.** Enter plan mode (`EnterPlanMode`), research read-only, and present the implementation plan AND the proposed fleet together via `ExitPlanMode` — that approval is **gate 1**. Skip plan mode for small/obvious changes.
...
- implement wave (`scoped-implementer`, sonnet) — each dispatch carries the plan section, decisions, and file list verbatim; agents cannot inherit this conversation (a `fork` can, but it runs on the main model, so reserve it for a short task whose input is the discussion itself)
- contextual safety check (`standards-reviewer`, opus) — only if the diff touched persistence/repository/controller/frontend or a migration/schema/model; NOT a full review
- simplification pass — after the implement wave, re-read the diff for reuse and abstraction: anything that duplicates an existing helper, adds a branch where an existing path could be extended, or bypasses the layer that owns the behaviour gets folded back in. Inline, or one `lens-reviewer` (sonnet) with the `smells/YAGNI/dead-code` + `reuse-and-abstraction` lenses if the diff is large. The goal is a smaller diff, not more code.
- investigate-before-fix (`investigator`, sonnet) for an ambiguous finding
- comment-density audit (`comment-auditor`, sonnet) — only if the wave added many comments
...
`scoped-implementer` · `lens-reviewer` · `standards-reviewer` · `adversarial-verifier` · `fix-verifier` · `comment-auditor` · `investigator` · `Explore`.

**No verification during development.** `adversarial-verifier` and `fix-verifier` belong to `/review`, run once when the change is complete. Development-time findings come only from the simplification pass, and they are resolved by refactoring into existing structure, never by adding a guard or a patch next to the code.
...
- **Ambiguous / exploratory** → `investigator` or `Explore` first, then decide.
...
- **Dispatch rules live here, not in CLAUDE.md.** Agents: `scoped-implementer` (sonnet), `lens-reviewer` (sonnet), `standards-reviewer` (opus), `investigator` (sonnet), `comment-auditor` (sonnet), `Explore` (sonnet); `adversarial-verifier` and `fix-verifier` are `/review`-only. Never Fable. Batch findings per wave, never one agent per item; cap a wave at ~5 agents; serial only on a real dependency, otherwise one concurrent wave.
...
- **Explicit model per agent, set at dispatch** — sonnet workhorse, opus for adversarial/standards judgment; never Fable. (Agent files don't pin a model so they also work in OpenCode; `CLAUDE_CODE_SUBAGENT_MODEL=sonnet` is the Claude-side safety net, not a substitute for setting it.)
- **Agents never touch git** — they leave the tree dirty; I commit. (Read-only agents block Write/Edit at the tool level, not the shell — "no git" holds by instruction.)
- Every dispatched agent reads the repo's CLAUDE.md / AGENTS.md and matches a cited sibling before writing. **Paste the relevant plan section and file list into the dispatch** so the agent doesn't rediscover them.
